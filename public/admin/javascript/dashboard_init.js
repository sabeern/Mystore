function loadSales() {
    return new Promise(async (resolve,reject) => {
        const salesArray = [],statusArray = [];
    const resultArray = await fetch('/admin/dashboard/monthlySale', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({  })
        }).then(res => res.json()).then(data => {
            const sale = data.monthlySales;
            for(let i=1;i<=12;i++) {
                let monthSale = 0;
                        sale.forEach((result) => {
                            if(result._id == i) {
                                monthSale = result.sales;
                            }
                        });
                        monthSale = (monthSale/1000).toFixed(2);
                    salesArray.push(monthSale);
            }
            let orderStatus = data.orderStatusCount;
            orderStatus.forEach((status) => {
                const statusType = status._id;
                const orderCount = status.count; 
                let eachStatus = {value:orderCount,name:statusType};
                statusArray.push(eachStatus);
            });
        });
        resolve({salesArray,statusArray});
    }); 
}
!(function (a) {
    "use strict";
            ...
            options: [
                {
                    ...
                    legend: { show: !1, x: "left", orient: "vertical", padding: 0, data: ["iPhone 11", "Windows", "Desktop77", "Mobiles", "Others"] },
                    toolbox: {
                        show: !0,
